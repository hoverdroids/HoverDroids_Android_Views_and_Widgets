...
import static com.hoverdroids.adapterview.view.TwoWayAbsListView.CHRIS_DEBUG;
...
public abstract class TwoWayAdapterView<T extends Adapter> extends ViewGroup {
	 ...
	 */
	@SuppressLint("WrongCall")//Chris added this because the call to onLayout was annoying
	...
	void rememberSyncState() {
		if(CHRIS_DEBUG)Log.d("TWAV","TWAV METHOD ENTER rememberSyncState");
		if (getChildCount() > 0) {
			mNeedSync = true;
			if (mIsVertical) {
				
				mSyncSize = mLayoutHeight;
				if(CHRIS_DEBUG)Log.d("TWAV","TWAV rememberSyncState mIsVertical=true, mSyncSize = mLayoutHeight:"+mLayoutHeight);
			} else {
				
				mSyncSize = mLayoutWidth;
				if(CHRIS_DEBUG)Log.d("TWAV","TWAV rememberSyncState mIsVertical=false, mSyncSize = mLayoutWidth:"+mLayoutWidth);
			}
			if (mSelectedPosition >= 0) {
				if(CHRIS_DEBUG)Log.d("TWAV","TWAV rememberSyncState mSelectedPositin >= 0; mSelectedPosition:"+mSelectedPosition);
				...
				if (mFirstPosition >= 0 && mFirstPosition < adapter.getCount()) {
					...
					if(CHRIS_DEBUG)Log.d("TWAV","TWAV rememberSyncState mFirstPosition>=0&&<adapter.getCount; mSyncRowId:"+mSyncRowId);
				} else {
					mSyncRowId = NO_ID;
					if(CHRIS_DEBUG)Log.d("TWAV","TWAV rememberSyncState mFirstPosition outOfBounds; mSyncRowId:"+mSyncRowId);
				}
				mSyncPosition = mFirstPosition;
				if(CHRIS_DEBUG)Log.d("TWAV","TWAV rememberSyncState mSyncPosition=mFirstPosition="+mFirstPosition);				
				if (v != null) {
					if(CHRIS_DEBUG)Log.d("TWAV","TWAV rememberSyncState v != null");
					if (mIsVertical) {
						mSpecificTop = v.getTop();
						if(CHRIS_DEBUG)Log.d("TWAV","TWAV rememberSyncState mIsVertical mSpecificTop=v.getTop+ndsco="+mSpecificTop);
					} else {
						mSpecificTop = v.getLeft();
						if(CHRIS_DEBUG)Log.d("TWAV","TWAV rememberSyncState mIsVertical=false mSpecificTop=v.getLeft+ndsco="+mSpecificTop);
					}
				}
				mSyncMode = SYNC_FIRST_POSITION;
				if(CHRIS_DEBUG)Log.d("TWAV","TWAV rememberSyncState mSyncMode = SYNC_FIRST_POSITION");
			}
		}
		if(CHRIS_DEBUG)Log.d("TWAV","TWAV METHOD EXIT rememberSyncState");
	}
