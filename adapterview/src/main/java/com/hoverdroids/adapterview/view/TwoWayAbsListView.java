...
ViewTreeObserver.OnTouchModeChangeListener {
	...
	/**
	 * Chris Sprague added
	 * The list will scroll to the desired position with the specified offset that is relative to that position.
	 * For example, set to position 3 with offset of 10, with children width of 100, would result in a total scroll
	 * of 350.
	 * 
	 * @see #setTranscriptMode(int)
	 */
	public static final int TRANSCRIPT_MODE_RELATIVE = 3;
	 ...
	 */
	public static final int SCROLL_VERTICAL = 0;//TODO chris sprague changed to public
	 ...
	 * @see #setScrollDirectionLandscape(int)
	 */
	public static final int SCROLL_HORIZONTAL = 1;//TODO chris sprague changed to public
	 ...
	 */
	protected int mWidthMeasureSpec = 0;//TODO chris changed to protected
...
//	//}
	...
	protected void onLayout(boolean changed, int l, int t, int r, int b) {
		if(CHRIS_DEBUG)Log.d("TWAV","TWALV METHOD ENTER onLayout");
		...
		if (changed) {
			...
			for (int i = 0; i < childCount; i++) {
				if(CHRIS_DEBUG)Log.d("TWAV","TWALV onLayout getChildAt("+i+").forceLayout");
		...
		if(CHRIS_DEBUG)Log.d("TWAV","TWALV METHOD EXIT onLayout");
	}
	...
	protected void layoutChildren() {
		if(CHRIS_DEBUG)Log.d("TWALV","TWALV METHOD ENTER layoutChildren");
		if(CHRIS_DEBUG)Log.d("TWALV","TWALV METHOD EXIT layoutChildren");		
	}
	...
	View obtainView(int position, boolean[] isScrap) {
		...
		View child;
		if (scrapView != null) {
			//Log.d("2wayLV","Not Null;Position:"+position);
			...
			//Log.d("2wayLV","Null;Position:"+position);
	...
	protected void handleDataChanged() {
		if(CHRIS_DEBUG)Log.d("TWALV","TWALV METHOD ENTER handleDataChanged");
		Log.d("twalv","twalv handleDataChanged");
		...
		if (count > 0) {
			...
			if (mNeedSync) {
				...
				if (mTranscriptMode == TRANSCRIPT_MODE_RELATIVE) {
					if(CHRIS_DEBUG)Log.d("TWALV","TWALV METHOD ENTER handleDataChanged transcripMode=set selection");
		...
		if(CHRIS_DEBUG)Log.d("TWALV","TWALV METHOD EXIT handleDataChanged");
	}
	 ...
	 *        in the recycler.
	 *
	 * @see com.library.twgv2.jess.ui.TwoWayAbsListView.RecycleBin
	 * @see com.library.twgv2.jess.ui.TwoWayAbsListView.RecyclerListener
	 */
	 ...
	 *
	 * @see com.library.twgv2.jess.ui.TwoWayAbsListView.RecycleBin
	 * @see com.library.twgv2.jess.ui.TwoWayAbsListView#setRecyclerListener(com.library.twgv2.jess.ui.TwoWayAbsListView.RecyclerListener)
	 */
	...
	abstract class TouchHandler {
		...
		//Todo chris added to allow external classes to stop the fling motion
		...
		/**
		 * Fires an "on scroll state changed" event to the registered
		 * {@link com.library.twgv2.jess.ui.TwoWayAbsListView.OnScrollListener}, if any. The state change
		 * is fired only if the specified state is different from the previously known state.
		 *
		 * @param newState The new scroll state.
		 */
	...
	class HorizontalTouchHandler extends TouchHandler {
		...
		public boolean onInterceptTouchEvent(MotionEvent ev) {
			...
			switch (action) {
				case MotionEvent.ACTION_DOWN: {
					if(CHRIS_DEBUG)Log.d("TWALV","TWGV onTouchIntercept action down");
					int touchMode = mTouchMode;
				...
				case MotionEvent.ACTION_MOVE: {
					if(CHRIS_DEBUG)Log.d("TWALV","TWGV onTouchIntercept action move");
					switch (mTouchMode) {
					case TOUCH_MODE_DOWN:
						final int x = (int) ev.getX();
				...
				case MotionEvent.ACTION_UP: {
					if(CHRIS_DEBUG)Log.d("TWALV","TWGV onTouchIntercept action up");
					mTouchMode = TOUCH_MODE_REST;
					mActivePointerId = INVALID_POINTER;
					reportScrollStateChange(OnScrollListener.SCROLL_STATE_IDLE);
					break;
		...
		public boolean onTouchEvent(MotionEvent ev) {
			...
			switch (action) {
				case MotionEvent.ACTION_DOWN: {
					if(CHRIS_DEBUG)Log.d("TWALV","TWGV onTouchEvent action down");
					final int x = (int) ev.getX();
					final int y = (int) ev.getY();
					int motionPosition = pointToPosition(x, y);
					if (!mDataChanged) {
				...
				case MotionEvent.ACTION_MOVE: {
					if(CHRIS_DEBUG)Log.d("TWALV","TWGV onTouchEvent action move");
					final int x = (int) ev.getX();
					deltaX = x - mMotionX;
					switch (mTouchMode) {
					case TOUCH_MODE_DOWN:
				...
				case MotionEvent.ACTION_UP: {
					if(CHRIS_DEBUG)Log.d("TWALV","TWGV onTouchEvent action up");
					switch (mTouchMode) {
					case TOUCH_MODE_DOWN:
					case TOUCH_MODE_TAP:
					case TOUCH_MODE_DONE_WAITING:
				...
				case MotionEvent.ACTION_CANCEL: {
					if(CHRIS_DEBUG)Log.d("TWALV","TWGV onTouchEvent action cancel");
					mTouchMode = TOUCH_MODE_REST;
					setPressed(false);
					View motionView = TwoWayAbsListView.this.getChildAt(mMotionPosition - mFirstPosition);
					if (motionView != null) {
	...
	//TODO chris added this so other classes can stop fling if desired
	public void endFling(){
